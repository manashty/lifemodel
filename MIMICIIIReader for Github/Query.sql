...
SELECT tmp.subject_id, tmp.hadm_id, tmp.admittime, tmp.dischtime, tmp.deathtime, tmp.dod, tmp.firstadmission, diags.icd9_code as diagICD9, diags.seq_num as diagSEQ, proceds.icd9_code as prodecICD9, proceds.seq_num as procedsSEQ  from tmp 
INNER JOIN DIAGNOSES_ICD diags on diags.hadm_id=tmp.hadm_id 
INNER JOIN PROCEDURES_ICD proceds on proceds.hadm_id=tmp.hadm_id